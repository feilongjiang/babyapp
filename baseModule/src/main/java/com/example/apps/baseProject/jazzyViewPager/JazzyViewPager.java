package com.example.apps.baseProject.jazzyViewPager;
...
//    }
    ...
    //        paramParcelable = (Bundle) paramParcelable;
            this.isFirst = ((Bundle) paramParcelable).getBoolean("value", true);
            super.onRestoreInstanceState(((Bundle) paramParcelable).getParcelable("super"));
            return;
        }
    ...
    public Parcelable onSaveInstanceState() {
        ...
        localBundle.putBoolean("value", this.isFirst);
        localBundle.putParcelable("super", localParcelable);
        return localBundle;
    }

    public void removeViewInObjMapByPosition(int paramInt) {
        mObjs.remove(paramInt);
    }
...
//	private int currentItem = 0;
    ...
    private static final float SCROLL_WIDTH = 20f, MAX_OFFSET = 300f;
    private LoadDataInterface loadData;
    ...
    public void setLoadData(LoadDataInterface loadData) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (getCurrentItem() == 0) {
                    if (offset > SCROLL_WIDTH) {//手指滑动的距离大于设定值
						...
						else if (!handleDefault && offset < -SCROLL_WIDTH) { //这种情况是已经出现缓冲区域了，手指慢慢恢复的情况, 防止抖动
                    ...
                    if (offset < -SCROLL_WIDTH) {
						...
						else if (!handleDefault && offset > SCROLL_WIDTH) {	//	防止抖动
    ...
    private void whetherConditionIsRight(float offset) {
        ...
        handleDefault = true;
        ILog.i("===left===", "relayout  offset is " + offset);
